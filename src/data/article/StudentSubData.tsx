import {SubContent} from "../ContentData.tsx";

export const StudentSubData: SubContent[] = [
    {
        title: 'โรคกระดูกพรุน [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'โรคกระดูกพรุน',
        path: '149-cancellous-bone-medical-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/9mo882JDST4" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>

<center class="contact-line">

ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link&nbsp;<a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>

</center>


<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360">
<br>
<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'ภาวะการติดเชื้อในโรคกระดูกและข้อ [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'ภาวะการติดเชื้อในโรคกระดูกและข้อ',
        path: '151-bone-joint-infect-medical-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/Anc0m_OGaUI" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link&nbsp;<a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360"> <br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'การตรวจร่างกายออร์โธปิดิกส์ [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'การตรวจร่างกายออร์โธปิดิกส์',
        path: '152-physical-exam-medical-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/05zx-13iDGY" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"><span data-mce-type="bookmark" style="display: inline-block; width: 0px; overflow: hidden; line-height: 0;" class="mce_SELRES_start">﻿</span></iframe></center>

<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link&nbsp;<a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>

<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360"> <br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'การฉีดยา steroid (Shoulder injection) [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'การฉีดยา steroid (Shoulder injection)',
        path: '153-shoulder-injection-medical-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/4YkLnD6dB7I" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link&nbsp;<a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360"> <br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'Direct lateral approach [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'Direct lateral approach',
        path: '170-direct-lateral-approach-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/QVTNDPSreKY" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link&nbsp;<a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360"> <br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'การผ่าตัดเปลี่ยนข้อสะโพกเทียม [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'การผ่าตัดเปลี่ยนข้อสะโพกเทียม',
        path: '169-hip-joint-surgery-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/LMLvy-q5rnM" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"><span data-mce-type="bookmark" style="display: inline-block; width: 0px; overflow: hidden; line-height: 0;" class="mce_SELRES_start">﻿</span></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link <a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360" /><br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'การผ่าตัดเปลี่ยนข้อเข่าเทียม [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'การผ่าตัดเปลี่ยนข้อเข่าเทียม',
        path: '168-knee-change-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/q-wMVOQw9s8" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"><span data-mce-type="bookmark" style="display: inline-block; width: 0px; overflow: hidden; line-height: 0;" class="mce_SELRES_start">﻿</span></iframe></center>
<center class="contact-line">        
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link <a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360" /><br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'Thompson test for Achilles tendon rupture [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'Thompson test for Achilles tendon rupture',
        path: '167-thompson-test-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/nDnhvxhUWaA" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link <a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360" /><br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'การฉีดยา steroid เข้าข้อเข่า (Knee Injection) [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'การฉีดยา steroid เข้าข้อเข่า (Knee Injection)',
        path: '155-knee-injection-medical-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/J8Ai3KBHIDY" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link <a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360" /><br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'Plantar fascia Injection [สำหรับแพทย์/นักศึกษาแพทย์]',
        shot: 'Plantar fascia Injection ',
        path: '156-plantar-fascia-injection-medical-km',
        detail: `<center><iframe src="https://www.youtube.com/embed/RRyzotUJ31I" width="560" height="315" frameborder="0" allowfullscreen="allowfullscreen"></iframe></center>
<center class="contact-line">
ผศ.นพ.ธนินนิตย์ ลีรพันธ์
Line ID: @doctorkeng
หรือกด Link <a href="https://line.me/R/ti/p/%40doctorkeng">https://line.me/R/ti/p/%40doctorkeng</a>
</center>
<br>
<img class="alignnone  wp-image-636 youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/12/doctorkeng.jpg" alt="" width="360" height="360" /><br>

<strong>อ่านความรู้โรคกระดูกและข้อเพิ่มเติมได้ที่</strong> <br/>

1. www.doctorkeng.com <br/>
2. https://doctorkeng.com.blogspot.com <br/>
3. https://www.facebook.com/backpainnonop <br/>
Email: tleerapun@gmail.com`
    },
    {
        title: 'CV หมอเก่ง',
        shot: 'ผศ.นพ.ธนินนิตย์ ลีรพันธ์',
        path: '157-cv-mor-keng',
        detail: `<img class="aligncenter wp-image-47 size-medium youtube-wrapper" src="https://doctorkeng.com/wordpress/wp-content/uploads/2018/01/ผศ.นพ.ธนินนิตย์-241x300.jpg" alt="" width="241" height="300" />
<p style="text-align: center;"><strong>ผศ.นพ.ธนินนิตย์ ลีรพันธ์</strong></p>
<strong>การศึกษา</strong>
<ul>
 \t<li>แพทยศาสตร์บัณฑิต คณะแพทยศาสตร์ มหาวิทยาลัยเชียงใหม่</li>
 \t<li>แพทย์เฉพาะทางออร์โธปิดิกส์ คณะแพทยศาสตร์ มหาวิทยาลบัยเชียงใหม่</li>
 \t<li>ปริญญาโทด้านงานวิจัยทางคลินิก Mayo Graduate School, Rochester, Minnesota, USA</li>
 \t<li>Research Fellow in Musculoskeletal Oncology, Mayo Clinic, USA</li>
 \t<li>ปริญญาโทด้านธุรกิจบัณฑิต มหาวิทยาลัยพายัพ</li>
 \t<li>Certificate of accomplishment in executive program “Systematic innovation of products, processes, and services” Massachusetts Institute of Technology Sloan School of Management, November 2014</li>
</ul>
<strong>การทำงาน</strong>
<ul>
 \t<li>แพทย์ผู้เชี่ยวชาญด้านออร์โธปิดิกส์</li>
 \t<li>ความชำนาญในเรื่องมะเร็งและเนื้อเยื่ออ่อนของกระดูกและข้อ</li>
 \t<li>การผ่าตัดเปลี่ยนกระดูกข้อสะโพกและข้อเข่าเทียม</li>
 \t<li>โรคกระดูกพรุน</li>
 \t<li>ผู้ประเมินรางวัลคุณภาพแห่งชาติ สำนักงานรางวัลคุณภาพแห่งชาติ</li>
 \t<li>ปี 2553(ตุลาคม)-2554(มีนาคม) ผู้ช่วยคณบดี คณะแพทยศาสตร์ มหาวิทยาลัยเชียงใหม่</li>
 \t<li>ปี 2554 (มีนาคม)-2556 รองผู้อำนวยการศูนย์ความเป็นเลิศ คณะแพทยศาสตร์ มหาวิทยาลัยเชียงใหม่</li>
 \t<li>ปี 2556-2560 รองคณบดี คณะแพทยศาสตร์ มหาวิทยาลัยเชียงใหม่ ด้านการจัดซื้อจัดจ้างและงานประชาสัมพันธ์</li>
 \t<li>ปี 2560-ปัจจุบัน ผู้อำนวยการศูนย์ศรีพัฒน์ คณะแพทยศาสตร์ มหาวิทยาลัยเชียงใหม่</li>
</ul>`
    },
    {
        title: 'Update for the treatment OA knee-นพ.ธนินนิตย์  ลีรพันธ์',
        shot: 'Update for the treatment OA knee',
        path: 'update-in-oa-treatment',
        detail: `<img class="aligncenter" src="http://www.lannastopdrink.com/wp-content/uploads/2019/02/update-in-OA-treatment_TaninnitLeerapun.jpg" alt="" width="700" height="" />

<!--[flipbook pdf="http://www.lannastopdrink.com/file-taninnit/update-in-OA-treatment-vertion-mobile_TaninnitLeerapun.pdf"]-->
<a href="http://www.lannastopdrink.com/file-taninnit/update-in-OA-treatment-vertion-mobile_TaninnitLeerapun.pdf" target="_blank" rel="noopener noreferrer">
<strong>หากต้องการดาวน์โหลด (สำหรับเครื่องคอมพิวเตอร์) ให้ปฏิบัติดังนี้</strong>
<br>
1. คลิก Link <a href="http://www.lannastopdrink.com/file-taninnit/update-in-OA-treatment_TaninnitLeerapun.pdf" target="_blank" rel="noopener">ดาวน์โหลด File PDF </a>หรือ <a href="http://www.lannastopdrink.com/file-taninnit/update-in-OA-treatment_TaninnitLeerapun.pptx" target="_blank" rel="noopener">ดาวน์โหลด File Power Point</a>
<br>2. เลือก Save Link As..
<br>3. กำหนดโฟลเดอร์ที่ต้องการจัดเก็บ
<br>4. กดปุ่ม Save`
    },
]